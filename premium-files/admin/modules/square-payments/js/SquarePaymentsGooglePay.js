(function ( $ ) {
	...
	$.WooSquare_Google_Pay_payments = {
		init: function() {
			// Checkout page
			$( document.body ).on( 'updated_checkout', function() {
				$.WooSquare_Google_Pay_payments.loadForm();	
			});
			...
			$( document.body ).on( 'click', '#WooSquare_Google_Pay_payments', function() {
				$( '.payment_box.WooSquare_Google_Pay_payments' ).css( { 'display': 'block', 'visibility': 'visible', 'height': 'auto' } );	
			});
		},
		loadForm: function() {
			
			if ( $( '#payment_method_square_google_pay' ).length ) {
				// work around for iFrame not loading if elements being replaced is hidden
				if ( ! $( '#payment_method_square_google_pay' ).is( ':checked' ) ) {
					$( '.payment_box.payment_method_square_google_pay' ).css( { 'display': 'block', 'visibility': 'hidden', 'height': '0' } );
				}
                ...
                callbacks: {
						cardNonceResponseReceived: function(errors, nonce, cardData, billingContact, shippingContact) {
							...
							if ( errors ) {
								...
								// append it to DOM
								$( '.payment_method_square_google_pay fieldset' ).eq(0).prepend( html );
				...
				$("#sq-google-pay").click(function(event){
				...
				$( 'form#order_review' ).on( 'submit', function( event ) {
					// remove any error messages first
					$( '.payment_method_square_google_pay .woocommerce-error' ).remove();

					if ( $( '#payment_method_square_google_pay' ).is( ':checked' ) && $( 'input.square-nonce' ).size() === 0 ) {
						payment_method_square_google_pay.requestCardNonce();
				...
				setTimeout( function() {
					if ( ! $( '#payment_method_square_google_pay' ).is( ':checked' ) ) {
						$( '.payment_box.payment_method_square_google_pay' ).css( { 'display': 'none', 'visibility': 'visible', 'height': 'auto' } );
					}
				}, 1000 );
			}
		}
	 ...
	 $.WooSquare_Google_Pay_payments.init();
}( jQuery ) );
